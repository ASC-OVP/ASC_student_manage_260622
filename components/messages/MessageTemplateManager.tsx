...
import { createMessageTemplateAction, deleteMessageTemplateAction, updateMessageTemplateAction } from "@/app/messages/actions";
    ...
    <section style={wrap}>
      <div style={head}>
        <div>
          ...
          <p style={desc}>운영 알림 문자만 관리합니다. 광고/홍보 문구는 이번 MVP 범위에서 제외됩니다.</p>
        </div>
      </div>
              ...
              <details key={template.id} style={item} open={false}>
                <summary style={summary}>
                  <span style={summaryMain}>
                    <b>{template.name}</b>
                    <small>{categoryLabel(template.category)} / {targetLabel(template.targetType)} / 수정 {formatDate(template.updatedAt)}</small>
                  </span>
...
const formGrid: CSSProperties = { display: "grid", gridTemplateColumns: "1.1fr 150px 150px 80px", gap: 8, alignItems: "start" };
const editGrid: CSSProperties = { display: "grid", gridTemplateColumns: "1.1fr 150px 150px 80px", gap: 8, marginTop: 8 };
